...
(function(window,jQuery,undefined){
	...
	function Tools(Browser){
		 ...
		 *			
		 */
		this.menu_of_auto_fixed=function(init){
			//获取要定位元素距离浏览器顶部的距离
			...
			$(window).scroll(function(){
				...
				if(scroH >= navH){
					...
					if(__this__.isFunction(fn1)) fn1(scroH);
				}else{
					if(css) $dom.removeClass(css);
					if(__this__.isFunction(fn2)) fn2(scroH);
				}
		 ...
		 *										         }
         * Example：
         *          this.hover_image($('.js_focus_image'),{'fn':this.show_toolBar_content});
         *----------------------------------*/
        this.hover_image=function($focus_image,init){
            var temp=null;
		...
		*----------------------------------*/
		this.disableRightMouse=function(){
			$(document).bind("contextmenu",function(e){  
			    return false;  
			});  
		};
		 ...
		 *----------------------------------*/
		this.enterToTab=function(inputs,event, input) {  
         ...
         * Example：
         *----------------------------------*/
        this.isFlashComplete=function(id){
         ...
         * Example：
         *----------------------------------*/
        this.getScale=function(i,a){return parseInt(i/a);};
